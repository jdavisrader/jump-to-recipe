...
### File storage
`src/lib/file-storage.ts` + `file-storage-config.ts` abstract storage backends, chosen in this order: **Vercel Blob** (`src/lib/blob-storage.ts`, active when `BLOB_READ_WRITE_TOKEN` or `BLOB_STORE_ID` is set — i.e. on Vercel), then S3 (`USE_S3=true`), then local disk under `public/uploads` (local dev / Docker). Blob keys mirror the local layout (`uploads/<category>/[recipeId/]<file>`). Recipe photos over 3.5 MB are downscaled in the browser (`src/lib/downscale-image.ts`, 2400px JPEG) before upload so they fit Vercel's ~4.5 MB request body limit — any new photo upload path must call `downscaleImageForUpload()`. Uploaded images route through `/api/upload` and `/api/images`; Sharp is used for optimization. `next.config.ts` has a long allowlist of remote recipe-site hostnames for `next/image` — add to `remotePatterns` when importing from a new source domain.
